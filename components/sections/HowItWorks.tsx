...
export function HowItWorks() {
  ...
  useEffect(() => {
    ...
    const setIndicator = (i: number) => {
      indicatorsRef.current.forEach((el, idx) => {
        if (!el) return;
        el.style.background =
          idx === i ? "rgb(255, 124, 97)" : "var(--line-strong)";
  ...
  return (
    ...
    >
      {/* progress bar */}
      <div className="absolute top-0 left-0 right-0 h-[2px] bg-line z-10 overflow-hidden">
        <div
          ref={progRef}
      ...
      <div className="relative z-10 mx-auto max-w-[1440px] px-6 lg:px-16 pt-16 lg:pt-20 pb-6 lg:pb-8 text-center">
        <div className="eyebrow text-fg/50 justify-center inline-flex">
          The method
        </div>
        <h2 id="how-it-works-heading" className="mt-4 font-grotesk font-bold text-fg text-display leading-[0.95]">
          How it <span className="italic text-accent">works.</span>
        </h2>
        <p className="mt-4 text-lg md:text-xl text-fg/60 max-w-md mx-auto leading-[1.55]">
      ...
      <div className="lg:hidden pt-6 pb-16">
        <div className="overflow-x-auto snap-x snap-mandatory no-scrollbar px-6">
          <div className="flex gap-5 w-max">
            {STEPS.map((step) => (
              <div
                key={step.num}
                className="snap-center shrink-0 w-[82vw] max-w-[380px] rounded-3xl bg-card border border-line shadow-[0_20px_40px_-24px_rgba(14,14,14,0.25)] p-6 flex flex-col items-start text-left"
              >
                <div
                  className="font-grotesk font-bold leading-none tracking-[-0.05em] text-fg"
                ...
                <h3
                  className="mt-2 font-grotesk font-bold text-fg"
                  style={{ fontSize: "clamp(1.6rem, 5.6vw, 2.25rem)", lineHeight: 1.04, letterSpacing: "-0.025em" }}
                >
                  {step.title}
                </h3>
                <p className="mt-3 text-[0.98rem] text-fg/60 leading-[1.55]">
                  {step.body}
                </p>
        ...
        <div className="mt-6 flex items-center justify-center gap-2 text-[0.68rem] uppercase tracking-[0.28em] font-semibold text-fg/40">
          <span>← Swipe →</span>
        </div>
      </div>
              ...
              <div className="relative h-[11vw] min-h-[128px] lg:min-h-[160px] mb-3">
                {STEPS.map((step, i) => (
                  <div
                    ...
                    className="absolute bottom-0 left-0 font-grotesk font-bold leading-[1.05] tracking-[-0.05em] text-fg whitespace-nowrap"
                ...
                {STEPS.map((step, i) => (
                  ...
                  >
                    <div className="text-accent text-[0.72rem] uppercase tracking-[0.22em] font-semibold mb-4 flex items-center gap-2">
                      <span className="w-[22px] h-px bg-current opacity-55" />
                      Step / {step.num}
                    </div>
                    <h3
                      className="font-grotesk font-bold text-fg"
                      style={{ fontSize: "clamp(2.5rem, 5.6vw, 5.6rem)", lineHeight: 1.02, letterSpacing: "-0.025em" }}
                    >
                      {step.title}
                    </h3>
                    <p className="mt-5 text-[1.28rem] text-fg/60 max-w-[520px] leading-[1.55]">
                      {step.body}
                    </p>
                  </div>
                ))}
              ...
              <div className="mt-8 flex items-center gap-2">
                {STEPS.map((_, i) => (
                  <span
                    ...
                    style={{
                      width: i === 0 ? 56 : 18,
                      background: i === 0 ? "rgb(255,124,97)" : "var(--line-strong)",
                    }}
                  />
                ))}
              </div>
